...
static sbuf_t *tryReadCompletePacket(buffer_stream_t *stream)
{
    ...
    {
        LOGW("PacketsToStream: UpStreamPayload: Not an IPv4 packet, skipping one byte");
...
static bool isOverFlow(buffer_stream_t *read_stream)
{
    ...
    {
        LOGW("PacketsToStream: UpStreamPayload: Read stream overflow, size: %zu, limit: %zu", bufferstreamGetBufLen(read_stream),
             kMaxBufferSize);
