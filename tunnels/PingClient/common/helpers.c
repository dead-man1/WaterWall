...
static bool pingclientPrepareInnerPacket(line_t *l, sbuf_t *buf, uint16_t *inner_packet_len_out)
{
    if (! pingclientValidateIpv4PacketBytes(sbufGetRawPtr(buf), sbufGetLength(buf), inner_packet_len_out))
    {
        return false;
    }
...
void pingclientEncapsulatePacket(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    if (! pingclientPrepareInnerPacket(l, buf, &inner_packet_len))
    {
        lineReuseBuffer(l, buf);
        return;
    }

    if (inner_packet_len > kPingClientMaxInnerPacketLength)
    {
        LOGW("PingClient: inner IPv4 packet exceeds ICMP tunnel size limit: %u > %u", inner_packet_len,
             (unsigned int) kPingClientMaxInnerPacketLength);
        lineReuseBuffer(l, buf);
        return;
    }
    ...
    IPH_LEN_SET(ipheader, lwip_htons((uint16_t) (inner_packet_len + kPingClientEncapsulationOverhead)));
...
void pingclientDecapsulatePacket(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    if (! pingclientValidateIpv4PacketBytes((const uint8_t *) sbufGetRawPtr(buf) + outer_header_len, inner_packet_len,
                                            &validated_inner_len))
    {
        tunnelPrevDownStreamPayload(t, l, buf);
        return;
