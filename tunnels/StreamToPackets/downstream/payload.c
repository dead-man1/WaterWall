...
static void localThreadStreamToPacketsTunnelDownStreamPayload(worker_t *worker, void *arg1, void *arg2, void *arg3)
{
    discard   worker;
...
void streamtopacketsTunnelDownStreamPayload(tunnel_t *t, line_t *l, sbuf_t *buf)
{
    ...
    if (ls->line == NULL)
    {
        for (wid_t wi = 0; wi < getWorkersCount(); wi++)
        {
            ...
            streamtopackets_lstate_t *wls = lineGetState(line, t);
            if (! wls->paused)
            {
                sendWorkerMessage(wi,(WorkerMessageCalback) localThreadStreamToPacketsTunnelDownStreamPayload,t,l,buf);
            }
        }
        lineReuseBuffer(l, buf);
        return;
